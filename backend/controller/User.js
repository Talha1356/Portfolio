...
// Helper function to destroy image safely
...
export const contact = async (req, res) => {
  try {
    ...
    console.log("Contact Controller", userMessage);
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (skills) {
      if (skills.image1) {
        await destroyImage(user.skills.image1.public_id);
      ...
      if (skills.image2) {
        await destroyImage(user.skills.image2.public_id);
      ...
      if (skills.image3) {
        await destroyImage(user.skills.image3.public_id);
      ...
      if (skills.image4) {
        await destroyImage(user.skills.image4.public_id);
      ...
      if (skills.image5) {
        await destroyImage(user.skills.image5.public_id);
      ...
      if (skills.image6) {
        await destroyImage(user.skills.image6.public_id);
    ...
    if (about) {
      ...
      if (about.avatar) {
        await destroyImage(user.about.avatar.public_id);
...
export const deleteYoutube = async (req, res) => {
  try {
    ...
    if (video) {
      await destroyImage(video.image.public_id);
    }
